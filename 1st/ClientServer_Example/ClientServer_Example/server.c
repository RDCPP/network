...
int main(int argc, char *argv[])
{
	...
	*/
	while(1) /* While server is not closed, client can connect this server by the socket. */
	{
		...
		/* And we need path to open file. */
		// int fd = open(path,O_RDONLY,0);

		FILE *f = fopen(path, "rb");

		/* In my
		 code, open does not work properly, so used fopen instead. */
		...
		if(f == NULL){
			...
			f = fopen("./src/404.html", "rb");
			/* If 404, we will show to client 404 page */
			write_to_fd(newsockfd, "Content-Type: text/html\r\n");
			write_to_fd(newsockfd, "Content-Length: ");
			int size = (int) fseek(f,0, SEEK_END);
			fseek(f, 0, SEEK_SET);
			char sizestr[20];
			...
			int n;
			while((n = fread(fb, 1, 1024, f)) > 0)
				write(newsockfd,fb,n);
			/* below open(), I copied else part (200 OK).
			   Detail Description of this part is else part, too. */
		}			
		else{
			...
			int total  =0;
			while((n = fread(fb, 1, 1024, f)) > 0) {
				n  = write(newsockfd,fb,n);
				if (n == -1) {
					perror("Error while writing data to stream");
					break;
				}
